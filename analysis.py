...
### THIS METHOD CREATE THE CORRELATION MATRIX ######################
...
#### THIS METHOD CALCULATE THE CONFIDENCE INTERVAL
...
# THIS METHOD CALL THE getMeanConfidenceInterval and print the confidence interval
...
# THIS METHOD RECEIVE EACH VARIABLE TO PLOT THE HISTOGRAM
...
#THIS METHOD RECEVEID EACH VARIABLE TO PLOT THE CORRELATION PLOTS
...
# THIS METHOD CREATE THE DENSITY PLOTS
def saveTwoKDE(df):
...
#THIS METHODS CREATE EVERY PLOT ONE METRIC VS TIME. ONE METRIC VS TEMPERATURE
def saveFigures(df):
    ...
    ls.append(['%.1f','TF', 'MTTF_SM', 'Temperature(°C)', 'Mean Temperature To Failure due to changes on thermal loads','Mean Temperature To Failure due to changes on thermal loads vs Temperature','plots/32_mean_time_to_failure_due_to_changes_on_thermal_loads vs temperature'])
    ls.append(['%.1f','TF', 'MTTFF_TC', 'Temperature(°C)', 'TC METRIC','TC METRIC vs Temperature','plots/33_tcmetric_vs'])
    ls.append(['%.4f','TF', 'A', 'Temperature(°C)', 'Availability','Availability','plots/34_availability_vs_time vs'])
    ls.append(['%.4f','TF', 'AEM', 'Temperature(°C)', 'Availability Due To Electromigration','Availability Due To Electromigration vs Temperature','plots/35_Availability Due To Electromigration vs temperature'])
    ls.append(['%.4f','TF', 'AC', 'Temperature(°C)', 'Availability Due To Corrosion','Availability Due To Corrosion vs Temperature','plots/36_Availability Due To corrosion vs temperature'])
    ls.append(['%.4f','TF', 'ATDDB', 'Temperature(°C)', 'Availability Due Temperature Depending Dielectric Breakdown','Availability Due Temperature Depending Dielectric Breakdown vs Temperature','plots/37_ Availability Due Temperature Depending Dielectric Breakdown vs temperature'])
    ls.append(['%.4f','TF', 'ASM', 'Temperature(°C)', 'Availability Due To Stress Migration','Availability Due To Stress Migration vs Temperature','plots/38 Availability Due To Stress Migration vs temperature'])
    ls.append(['%.4f','TF', 'ATC', 'Temperature(°C)', 'Availability Due To Thermal Cycling','Availability Due To Thermal Cycling vs Temperature','plots/39 Availability Due To Thermal Cycling vs temperature'])
    ls.append(['%.2f','TF', 'TAA', 'Temperature(°C)', 'Availability Due To External Temperature','Availability Due To External Temperature vs Temperature','plots/40 Availability Due To External Temperature vs temperature'])
    ls.append(['%.2f','TF', 'QRED', 'Temperature(°C)', 'Thermal Load Released','Thermal Load Released vs Temperature','plots/41 Thermal Load Released vs temperature'])    
    ls.append(['%.2f','TF', 'QR', 'Temperature(°C)', 'Power Required','Power Required vs Temperature','plots/42 Power Required vs temperature'])
    ls.append(['%.3f','TF', 'PUE', 'Temperature(°C)', 'Power Usage Efficiency','Power Usage Efficiency vs Temperature','plots/43 Power Usage Efficiency vs temperature'])
    ls.append(['%.2f','TF', 'DCie', 'Temperature(°C)', 'DCie','DCie vs Temperature','plots/44 DCie vs temperature'])
    ls.append(['%.4f','TF', 'cost', 'Temperature(°C)', 'Cost','Cost vs Temperature','plots/45 cost vs temperature'])
    ls.append(['%.2f','TF', 'MTTF_IC', 'Temperature(°C)', 'Unified Reliability','Unified Reliability','plots/46 Unified Reliability'])
    ls.append(['%.4f','TF', 'A_TC', 'Temperature(°C)', 'Unified Availability','Unified Availability vs Temperature','plots/47 Unified Availability vs temperature'])
    ls.append(['%.3f','TF', 'Q_DIT', 'Temperature(°C)', 'Amount Energy Dissipated','Amount Energy Dissipated vs Temperature','plots/48 Amount Energy Dissipated vs temperature'])
    ls.append(['%.2f','TF', 'TPF', 'Temperature(°C)', 'External Temperature Impact','External Temperature Impact vs Temperature','plots/25 External Temperature Impact vs Temperature'])
    ls.append(['%.4f','TF', 'AIRFLOW', 'Temperature(°C)', 'Required Volume Airflow','Required Volume Airflow vs Temperature','plots/26 Required Volume Airflow vs Temperature'])
    ls.append(['%.4f','TF', 'TAAF', 'Temperature(°C)', 'Thermal Accelerated Aging','Thermal Accelerated Aging vs Temperature','plots/27 Thermal Accelerated Aging vs Temperature'])
    ls.append(['%.3f','TF', 'DeltaT_de', 'Temperature(°C)', 'Temperature rise due to the dissipation of energy','Temperature rise due to the dissipation of energy vs Temperature','plots/28 Temperature rise due to the dissipation of energy vs Temperature'])
    ls.append(['%.3f','TF', 'QD', 'Temperature(°C)', 'Energy Demanded','Energy Demanded vs Temperature','plots/29 Energy Demanded vs Temperature'])
...
#THIS METHOD CALL N TIMES TO THE HISTOGRAM PLOTS TO DO THIS PLOTS
...
#THIS METHODS CALL TO EVERY CORRELATION PLOTS
...
############# HERE WE ARE CALLING EVERY FUNCTION ##########
df = load_csv()
table = df.iloc[:,41:]
saveTable(table.describe().round(4), 'Variables', 'Variables describe')
saveHeatMap(table, 'Heatmap', 'Correlation matrix')
saveTwoKDE(df)
saveFigures(df)
saveGroupPlot(df)
