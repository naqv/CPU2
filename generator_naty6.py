...
def get_average_cpu_freceuncy(dataframe):
    ec_1 = (dataframe['vnf cpu usage'] * NCORES * 2.3)/100
    return ec_1

#equation 2
def output_frequency(vectorFrequency,room_temperature):
    #temp value between 20 and 30
    #freq in MHz, it does not need transformation
    #Eq = roomTemp + a * (1 - e^b) where
    #a = (I * V + (alpha * Cp * V2 * freq)) / (h * As)
    #b = - (3600 * (h * As)/m*C) * t
...
#equation 3
def mttf(temperature,room_temperature):
    #Equation
    #MTTF_tp = MTTF_TPO * e^(E_a/k * (1/T_p - 1/T_po))
    #REMEMBER T_P AND T_po are measured in Kelvin
    #not fixed yet
    mtt= mttftpo*(math.e**((e0/k)*(1/(temperature + 273) - 1/(np.array(room_temperature) + 273))))
    return mtt
...
def power_usage_efficiency(frecuency, room_temperature):
    ...
    b_ = (3.792 + SEER) * (fa * (1 - nt )) + 3.792 * (h * as_motherboard * (np.array(room_temperature) - td) )
    c_ = ne * SEER
    PUE = fa / ( (b_/c_) * te)
    return PUE

#need to check
def DCie(frecuency, room_temperature):
    fa = (I * V) + (cp * activity_factor * (V **2) * frecuency)
    c_ = ((3.792 + SEER) * (fa * (1 - nt))) +  (3.792 * (h * as_motherboard * (np.array(room_temperature) - td )))
    d_ = fa * ne * SEER
    DCie=  100 * ( c_ /d_ ) * te
    return DCie
...
#new (I.A.)
def volumeAirflow(frecuency, room_temperature):
    fac = (I * V) + (cp * activity_factor * (V **2) * frecuency)
    a_ = ( fac * (1 - nt) + ( h * as_motherboard * (np.array(room_temperature) - td)) ) * te
    b_ = fac/(h * as_motherboard)
    c_ = 1 - math.e**( ((-3600 * h * as_motherboard )/(mass * C)) * t )
    d_ = (fac * (1 -nt) )/ (h * as_motherboard)
...
def main():
    try:
        ...
        with mp.Pool(processes = 27) as pool:
            r1 = pool.apply_async(mttf,([tf,temperature_room]))
            ...
            r17 = pool.apply_async(power_usage_efficiency,([freq,temperature_room]))
            r18 = pool.apply_async(DCie,([freq,temperature_room]))
